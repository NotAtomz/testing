...
if ANTI_CRASH_DISABLED then
	warn('made by ERROR NAME WAS CHANGED)')
end
...
ScorpionAdmin.Name = "tubers Admin"
...
Title_3.Text = "tubers Admin-Loader "..version
...
Title.Text = "tubers Admin-Commands"
...
Title_2.Text = "tubers Admin-Credit"
...
TextLabel_2.Text = "Made By ERROR NAME CHANGED By ERROR NAME CHANGED"
...
Title_5.Text = "tubers Admin-Output"
...
function LocalPlayerChatted(msg)
	...
	if command("brag") then
		chat("a head admin is in the server! please respect him and dont brag for admin.")
	...
	if command("admin") then
		...
		if player and not admins[player.UserId] then 
			...
			chat(player.Name..", a head admin has ranked you as a temp admin. Type "..prefix.."cmds to see all commands for you!")
		end
	...
	if command("unadmin") then
		...
		chat(player.Name..", your admin has been remove by abuse of admin.")
...
function AdminPlayerChatted(msg,player)
	...
	if command("cmds") then
		...
		chat("/w "..player.Name.." sorry but devs could not find a cmd to add more cmds will be added in the next update! you can leave and rejoin the server and as long as the head admin is here, you will retain your access!")end
	if command("loopkill") or command("lk") then
		local player = GetPlayerFromString(arg2)
...
getgenv().UNLOAD_ADMIN = function() 
	...
	warn("Successfully unloaded head admin!") 
end 
